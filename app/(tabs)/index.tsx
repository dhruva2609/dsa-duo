      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        {completedLevels.length > 0 && (
          <>
            ...
            {topics
              .filter((topic) => isTopicCompleted(slugify(topic.topic)))
              .map((topic) => {
                const slug = slugify(topic.topic);
                const iconData = topicIcons[slug] || { name: 'book', color: '#6366F1' };
                return (
                  <TouchableOpacity key={`continue-${slug}`} style={[styles.card, { backgroundColor: colors.card }]} onPress={() => setSelectedTopic(slug)}>
                    ...
                    <View style={styles.textContainer}>
                      ...
                      <Text style={[styles.cardSubtitle, { color: colors.text }]}>Keep the streak alive!</Text>
                    </View>
        ...
        {topics.map((topic) => {
          ...
          return (
            <TouchableOpacity key={slug} style={[styles.card, { backgroundColor: colors.card }]} onPress={() => setSelectedTopic(slug)}>
              ...
              <View style={styles.textContainer}>
                <Text style={[styles.cardTitle, { color: colors.text }]}>{topic.topic}</Text>
                <Text style={[styles.cardSubtitle, { color: colors.text }]}>5 Lessons</Text>
              </View>
              <MaterialCommunityIcons name="chevron-right" size={24} color={colors.text} />
            </TouchableOpacity>
          );
...
const styles = StyleSheet.create({
  ...
  choiceBtn: { padding: 16, borderRadius: 14, alignItems: 'center', marginBottom: 12 },
